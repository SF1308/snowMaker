# Technical: stack, structure and how to run the project

> This page does not assume any knowledge about snowmaking. If you are interested in the domain (what snowmaking is, how it works) or the equation details, see **Snowmaking** and **Architecture**.

## Stack

- **Python 3.10+** (uses `float | None` syntax, available since 3.10)
- **Streamlit** — UI framework
- **Plotly** — charts (wet bulb gauge, production/quality/energy visuals)
- **dataclasses** (stdlib) — data modeling
- **math** (stdlib) — trigonometric functions for the Stull formula

> 🔧 **Pending:** there is an `i18n/` module scaffolded (ES/EN toggle) but not yet integrated into `app.py`, `ui/weather.py`, or `ui/snowgun.py`. See "Known issues" below.

The domain logic (`calculators/`, `engine/`, `models/`) does not depend on Streamlit or any external library — only `app.py` and `ui/` do. This is intentional: it allows testing or reusing the calculation engine without needing to launch a UI.
...
It calculates the wet bulb for fixed conditions (currently hardcoded in `main.py`) and prints it to the console. Useful for quick formula debugging without launching Streamlit.

> ⚠️ Currently `main.py` has a broken import (see the "Known issues" section below).

### Codespaces / Devcontainer

The project includes `.devcontainer/devcontainer.json` based on `python:3.11-bookworm`. Opening it in GitHub Codespaces installs dependencies automatically and starts the app on port `8501` with preview enabled.
...
Each `calculator` answers **one question** and does not know about the others. `WetBulbCalculator` does not know `ViabilityCalculator` exists — it returns a `float` and that’s it. `SnowEngine` is the only component that knows the dependency order and chains outputs from one step as inputs to the next:
...
If these values change, modify only the responsible calculator — there is no duplication in other layers.
